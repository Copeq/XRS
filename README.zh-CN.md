...
XRS 是一个面向树莓派和其他 Linux 采集节点的固定式 Remote ID / OpenDroneID Wi-Fi 监测器。
...
| `xrs_scanner/host_metrics.jsonl` | 主机负载采样数据 | 是（系统临时目录） |

`--config` 参数可指向任意路径：

```bash
python run.py --config /etc/xrs/config.json --no-tui
```
...
- 运行时修复卡片可以创建 / 确认 `rid` 专用运行账号、授予采集和热点能力、安装无线工具，并注册 / 更新 `xrs-scanner.service`。
...
节点负载默认关闭。开启后数据约每分钟采样一次，默认写入系统临时目录下的 `xrs_scanner/host_metrics.jsonl`。文件不存在会自动创建，服务重启后继续沿用，并按 `metrics.retention_days` 自动裁剪。
...
启用后，扫描器约每分钟采样一次主机指标，存储在系统临时目录的 `xrs_scanner/host_metrics.jsonl` 中。文件自动创建，服务重启后继续沿用，按 `metrics.retention_days`（默认 7 天）自动裁剪。
...
| `station_edition/xrs/` | 全部扫描、解析、服务、鉴权、UI 模块 | 是 |
