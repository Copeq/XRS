...
<header>
  <h1>✈ XRS</h1><code class="app-version-label">__APP_VERSION_LABEL__</code>
...
function addNotificationEntry(text, kind){
  ...
  fetch(apiUrl('/api/notifications'), {
    method:'POST',
    cache:'no-store',
    headers:{'Content-Type':'application/json','X-XRS-Page':'1'},
...
async function requestWebNotifyPermission(){
  ...
  try{
    ...
    if(webNotifyEnabled){
      try{
        new Notification('XRS 通知已启用', {body:'将推送飞机上下线事件'});
      }catch(_e){}
...
async function getJson(url){
  var resp = await fetch(apiUrl(url), {cache:'no-store', headers:{'X-XRS-Page':'1'}});
  var data = {};
...
async function getJson(url){
  const r = await fetch(apiUrl(url), {cache:'no-store', headers:{'X-XRS-Page':'1'}});
...
async function postJson(url, body){
  const r = await fetch(apiUrl(url), {method:'POST', headers:{'Content-Type':'application/json','X-XRS-Page':'1'}, body:JSON.stringify(body||{})});
  const d = await r.json().catch(()=>({}));
  if(authExpired(r, d)){ redirectLogin(); throw new Error('login required'); }
  if(!r.ok || d.ok===false) throw new Error(d.error || ('HTTP '+r.status));
  return d;
...
.simulation-modal{position:fixed;top:88px;right:24px;z-index:10020;display:none;padding:0;background:transparent;box-shadow:none}
.simulation-modal.show{display:block}
.simulation-card{width:min(420px,92vw);max-height:calc(100dvh - 120px);overflow:auto;border:1px solid var(--border);border-radius:18px;background:var(--panel);box-shadow:var(--shadow-lg)}
...
(function(){
  ...
  function detailReparseApiUrl(){
    return String(window.XRS_DETAIL_REPARSE_API || '/api/history/reparse');
  }
  ...
  function modelIssueUrl(sn, prefix){
    ...
    return 'https://github.com/Copeq/XRS/issues/new?title='
      + encodeURIComponent(title) + '&body=' + encodeURIComponent(body);
  }
  function modelPrEditUrl(){
    return 'https://github.com/Copeq/XRS/edit/main/rid_model.json';
  }
...
<main class="card">
  <h1 class="brand">XRS</h1>
  <p class="desc">登录 XRS</p>
...
function qs(id){{ return document.getElementById(id); }}
function pageHeaders(extra){{ var h={{'X-XRS-Page':'1'}}; if(extra) Object.keys(extra).forEach(function(k){{ h[k]=extra[k]; }}); return h; }}
...
def _station_asset_roots() -> list[Path]:
    ...
    if frozen_root:
        roots.append(Path(frozen_root) / "station_edition" / "xrs")
    roots.append(Path.cwd() / "station_edition" / "xrs")
    roots.append(Path.cwd() / "xrs")
...
def _build_mobile_html() -> str:
    ...
    if template_path is None:
        return '<!doctype html><html lang="zh"><head><meta charset="utf-8"><title>XRS Mobile</title></head><body>mobile template missing</body></html>'
    try:
...
def _build_settings_html() -> str:
    ...
    if template_path is None:
        return '<!doctype html><html lang="zh"><head><meta charset="utf-8"><title>XRS</title></head><body>settings template missing</body></html>'
    try:
        html_src = template_path.read_text(encoding="utf-8")
    except OSError:
        return '<!doctype html><html lang="zh"><head><meta charset="utf-8"><title>XRS</title></head><body>settings template missing</body></html>'
