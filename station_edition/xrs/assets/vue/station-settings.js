(() => {
  ...
  async function downloadQualityReport() {
    ...
    var filename = parseFilenameFromDisposition(r.headers.get("Content-Disposition")) || "xrs-quality.zip";
  ...
  async function registerSystemdServiceFromSettings() {
    if (!confirm("将写入 /etc/systemd/system/xrs-scanner.service，并启用开机自启。继续吗？")) return;
      ...
      const rsp = await withSettingsAsyncTask("更新上传", "正在上传并校验安装包...", function() {
        return requestJson("/api/settings/app-update/upload", {
          ...
          headers: pageHeaders({
            ...
            "X-XRS-Upload-Name": encodeURIComponent(String(file.name || "package.bin")),
            "X-XRS-Upload-Token": String(appUpdateUploadMeta && appUpdateUploadMeta.token || "")
          }),
          body: file
        });
      });
  ...
  async function createPasskeyWithCreds() {
    ...
    var createOptions = {
      publicKey: {
        challenge,
        rp: pk.rp || { name: start.realm || "XRS", id: start.rp_id || location.hostname },
        user: {
          id: userId,
  ...
  function collectNetworkBindings() {
    ...
    if (qs("net-ap-ssid")) ap.ssid = v("net-ap-ssid") || "XRS-HotSpot";
  ...
  function renderNetworkBindings() {
    ...
    if (qs("net-ap-ssid")) qs("net-ap-ssid").value = String(ap.ssid || "XRS-HotSpot");
  ...
  async function loadVisual(opts) {
    ...
    qs("cfg-auth-realm").value = String(auth.realm || "XRS");
