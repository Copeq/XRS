#!/usr/bin/env python3
"""Build and Raspberry Pi sync helper for XRS editions."""
...
def build_binary(
    ...
    *,
    ...
    env = dict(os.environ)
    env["XRS_EDITION"] = edition
    env["XRS_TARGET"] = target
    if release_tag:
    ...
    cmd = [
        ...
        "_sqlite3",
        "--add-data",
        data_arg("station_edition/xrs", "station_edition/xrs"),
        "--add-data",
        data_arg(
            "station_edition/xrs/resources/rid_model.json",
            "station_edition/xrs/resources",
        ),
        "--add-data",
...
def sync_pi(artifact: Path, *, config: str, restart: bool) -> None:
    ...
    env = dict(os.environ)
    env["XRS_PREBUILT_BINARY"] = str(artifact)

    run(args, env=env)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build XRS edition binaries")
    p.add_argument(
        "--edition",
