...
from station_edition.xrs.analize_core import parse_raw_packet, parse_rid_payload, parse_rid_payloads
from station_edition.xrs.runtime import create_runtime_context, load_namespace
...
def parser_namespace():
    return load_namespace(
        create_runtime_context(
            ...
            module_name="station_edition.xrs._test_parser_roles",
        )
    )
...
def common_namespace():
    return load_namespace(
        create_runtime_context(
            ...
            module_name="station_edition.xrs._test_common_tracks",
        )
    )
...
def process_namespace():
    return load_namespace(
        create_runtime_context(
            ...
            module_name="station_edition.xrs._test_process_tracks",
        )
    )
...
class RidParserRoleTests(unittest.TestCase):
    ...
    def test_frontend_map_uses_role_specific_fields(self):
        source = Path("station_edition/xrs/web_server.py").read_text(encoding="utf-8")
    ...
    def test_first_startup_creates_config_and_sqlite_store_when_paths_are_missing(self):
        ns = startup_namespace("station_edition.xrs._test_first_startup_default")
    ...
    def test_first_startup_with_locked_missing_config_uses_defaults_and_creates_sqlite_store(self):
        ns = startup_namespace("station_edition.xrs._test_first_startup_locked")
