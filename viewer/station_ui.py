...
(function(){
  ...
  function patchTitle(){
    try{ document.title = 'XRS Node Center'; }catch(_e){}
    var title = document.querySelector('header h1');
    if(title) title.textContent = 'XRS Node Center';
...
def build_station_viewer_page() -> str:
    ...
    # 全量品牌替换一次：XRS -> XRS Node Center（含 <title>），避免重复追加
    html_src = html_src.replace("XRS", "XRS Node Center")
    viewer_css = """
