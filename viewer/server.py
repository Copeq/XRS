"""HTTP entrypoint for the standalone XRS node-center viewer."""
...
def _viewer_send_wecom(store: ConfigStore, text: str) -> tuple[bool, str]:
    ...
    req = urllib.request.Request(
        ...
        data=body,
        headers={"Content-Type": "application/json", "User-Agent": f"XRSViewer/{APP_VERSION}"},
        method="POST",
    )
    try:
...
def _maybe_notify_viewer_node_status(store: ConfigStore, snap: dict[str, Any]) -> None:
    ...
    for node in nodes:
        ...
        _viewer_send_wecom(store, f"XRS Viewer · {name} {status}\n{detail}")
...
document.getElementById('accept').onclick = async function(){{
  ...
  const r = await fetch('/api/eula/accept', {{method:'POST', headers:{{'Content-Type':'application/json','X-XRS-Page':'1'}}, body:'{{}}'}});
...
class ViewerHandler(BaseHTTPRequestHandler):
    server_version = "XRSViewer/" + APP_VERSION
    sys_version = ""
    store: ConfigStore
    listen_label: str = ""
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/settings/notify/test":
                ok, msg = _viewer_send_wecom(self.store, "XRS Viewer 通知测试")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(description="XRS node-center viewer")
