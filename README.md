...
│   └── xrs/
│       ├── app.py                   # Bootstrap: loads ordered domain chunks (see runtime.py)
│       ├── runtime.py               # DEFAULT_CHUNK_FILES ordering + shared-namespace loader
│       ├── common_core.py           # Foundation: constants, global state, logging, EULA/utils
│       ├── process_core.py          # Aircraft state machine / live tracking core
│       ├── auth_core.py             # Auth policy gate, sessions, rate limits, SSO
│       ├── hardware_core.py         # Command/privilege/`iw` low-level helpers
│       ├── scan_core.py             # Wi-Fi capture glue (forwards decoding to analize_core)
│       ├── analize_core.py          # Standalone RID/GB46750 parser core
│       ├── network_binding_core.py  # NIC role assignment / AP binding coordination
│       ├── web_server.py            # Page HTML/CSS/JS templates + page builders
│       ├── web_http_core.py         # HTTP+WebSocket do_GET/do_POST dispatch
│       ├── cli_app.py               # CLI entry, args, restart orchestration
│       ├── platform_compat.py       # Platform-specific compatibility layer
│       └── *_core.py                # ~30 domain chunks split out of the monoliths above:
│                                    #   track_store, history_storage, history_lifecycle,
│                                    #   settings_io, config, security, runtime_init,
│                                    #   notify, oui_ap, models_remote, systemd_iw,
│                                    #   sniff_iface, live_ws, app_update, history_process,
│                                    #   process_scan_diff, api_payload, webauthn, host_hw,
│                                    #   ap_dhcp_dns, ap_hostapd, cli_parse, cli_tui ...
│                                    #   Full authoritative list lives in runtime.py.
├── portable_edition/
...
The station runtime uses a chunk-loading architecture: `app.py` loads and executes the files listed in `runtime.py: DEFAULT_CHUNK_FILES` (foundation chunk first, then domain chunks, then `scan_core`/`process_core`/`simulation_core`/`auth_core`/`network_binding_core`/`web_server`/`web_http_core`/`cli_app`), assembling them into a single shared namespace at runtime. The former giant files (`common_core`, `process_core`, `auth_core`, `hardware_core`, `web_server`) were split into focused domain chunks during a backend refactor, so individual files stay readable while the build process remains unchanged (PyInstaller ships the whole `xrs/` directory).
...
```bash
install -m 0755 xrs_station-arm64 /opt/xrs/xrs_station-arm64
```
...
[Service]
ExecStart=/opt/xrs/xrs_station-arm64 --config /opt/xrs/config.json --no-tui
Restart=always
RestartSec=10
...
| `xrs_scanner/host_metrics.jsonl` | Host load samples | Yes (system temp dir) |
...
When enabled, the scanner samples host metrics approximately once per minute and stores them in the system temp directory under `xrs_scanner/host_metrics.jsonl`. The file is created automatically, preserved across service restarts, and pruned according to `metrics.retention_days` (default: 7 days).
...
**Default source:**
```
https://raw.githubusercontent.com/Copeq/XRS/refs/heads/main/rid_model.json
```
...
| `station_edition/xrs/` | All scanner, parser, server, auth, and UI modules | Yes |
